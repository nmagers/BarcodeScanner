...
interface BarcodeDao {
    ...
    @Query("SELECT * FROM barcodes WHERE value = :value AND unit = :unit LIMIT 1")
    suspend fun findByValueAndUnit(value: String, unit: String): BarcodeEntry?
    ...
    @Query("UPDATE barcodes SET productName = :name, productSize = :size, unit = :unit, quantity = :quantity, needsReview = :needsReview WHERE id = :id")
    suspend fun updateUserFields(id: Long, name: String?, size: String?, unit: String, quantity: Int, needsReview: Boolean)
